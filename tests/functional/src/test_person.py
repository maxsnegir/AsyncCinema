...
def redis_person_params(settings, query_person_params) -> str:
    return settings.PERSON_INDEX + ":None:" + ":".join(str(value) for value in query_person_params.values())
...
class TestPerson:
    ...
    async def test_person_list_without_params(
        ...
        assert response.body == response_with_page.body, "Запрос без параметров должен соответствовать запросу с параметрами по умолчанию"
        assert await redis_client.get(redis_person_params),'Отсутствуют данные в redis'

    @pytest.mark.asyncio
    async def test_person_page_number_param(
            self,create_test_data,make_get_request,query_person_params,redis_person_params,redis_client):
    ...
    async def test_person_by_id(self,create_test_data,make_get_request,settings,redis_client):
        ...
        assert await redis_client.get(settings.PERSON_INDEX + ":" + existing_person), 'Отсутствуют данные в redis'
